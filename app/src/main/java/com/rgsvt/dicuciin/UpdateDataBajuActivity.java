...
public class UpdateDataBajuActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_update_data_baju);
        ...
        String sql = "SELECT * FROM cucibaju WHERE no_cucibaju='"+ getIntent().getStringExtra("no_cucibaju") +"' LIMIT 1";
        cursor = db.rawQuery(sql, null);
        Log.d("data", "select cucibaju="+cursor);
        ...
        btnEdit.setOnClickListener(new View.OnClickListener(){
            ...
            public void onClick(View arg0){
                ...
                if (edit.isEmpty()) {
                    ...
                    String sql = "update cucibaju set nama_pelanggan='" + nama_pelanggan.getText().toString() + "', jenis_cuci='" + jenis_cuci.getText().toString() + "', jenis_kategori='" + jenis_kategori.getText().toString() + "', harga_kg='" + harga.getText().toString() + "', berat='"+berat.getText().toString()+"', total_harga='"+total.getText().toString()+"', bayar='"+bayar.getText().toString()+"' where no_cucibaju='"+getIntent().getStringExtra("no_cucibaju")+"'";
                    db.execSQL(sql);
                    Toast.makeText(getApplicationContext(), "Data Cucian Terubah!", Toast.LENGTH_LONG).show();
                    finish();
                }
                DataLaundryBajuActivity.dlba.refreshList();
            }
        });
